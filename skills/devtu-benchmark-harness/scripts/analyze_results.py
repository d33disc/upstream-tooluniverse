...
def analyze(results_path: str, benchmark: str = "") -> dict:
    ...
    for config_name, results in all_configs.items():
        ...
        by_category: dict[str, dict[str, Any]] = defaultdict(
            lambda: {"correct": 0, "total": 0, "failures": []}
        )
        for r in results:
...
def print_analysis(analysis: dict):
    ...
    for config_name, data in analysis.items():
        ...
        print(f"\n{'=' * 60}")
        print(
            f"{config_name}: {overall['correct']}/{overall['total']} ({overall['accuracy']}%)"
        )
        print(
            f"Total time: {overall['total_time']}s, avg: {overall['avg_time']}s/question"
        )
        print(f"{'=' * 60}")
