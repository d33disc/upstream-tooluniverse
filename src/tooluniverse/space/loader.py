...
class SpaceLoader:
    ...
    def _resolve_extends(
        ...
        """
        Resolve the ``extends`` field by loading the base Space and merging.

        The base Space is loaded first; the current config is then deep-merged
        ...
        if not isinstance(base_uri, str):
            raise ValueError(
                f"Space 'extends' must be a string URI, "
        ...
        if base_uri in _chain:
            raise ValueError(
                f"Circular Space extends detected: '{base_uri}' appears more than "
                f"once in the inheritance chain {sorted(_chain)}"
            )

        try:
            # Pass the expanded chain into the recursive load so every level
            # can detect cycles regardless of how deep the chain is.
            raw = self._load_raw(base_uri)
            raw = self._resolve_extends(raw, _chain | {base_uri})
        except ValueError:
            raise
        except Exception as e:
            raise ValueError(f"Failed to load base Space '{base_uri}': {e}")
    ...
    def resolve_to_local_dir(self, uri: str) -> Path:
        """
        Resolve any URI to a local directory that contains Space tool files.
    ...
    def _resolve_local_to_dir(self, uri: str) -> Path:
        ...
        p = Path(uri)
        if not p.exists():
            raise ValueError(f"Space path not found: {uri}")
    ...
    def _resolve_http_file_to_dir(self, uri: str) -> Path:
        ...
        filename = uri.rstrip("/").split("/")[-1] or "space.yaml"
        ...
        if not cached_file.exists():
            try:
                ...
                raise ValueError(f"Failed to download Space from URL {uri}: {e}")
    ...
    def get_tool_files_from_dir(
        ...
        ``space.yaml`` / ``space.json`` are excluded (handled separately).
    ...
    def _load_raw(self, uri: str) -> Dict[str, Any]:
        """Load and parse a Space URI without validation or extends resolution."""
        if uri.startswith("hf:"):
        ...
        if not isinstance(result, dict):
            raise ValueError(
                f"Space configuration at '{uri}' must be a YAML/JSON mapping, "
    ...
    def _deep_merge(base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in override.items():
            if key in result and isinstance(result[key], dict) and isinstance(value, dict):
                result[key] = SpaceLoader._deep_merge(result[key], value)
            else:
                result[key] = value
        return result
