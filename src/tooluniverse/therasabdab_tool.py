...
class TheraSAbDabTool(BaseTool):
    ...
    def _request(self, method: str, url: str, **kwargs) -> requests.Response:
        """Retry transient Thera-SAbDab gateway failures."""
        kwargs.setdefault("timeout", self.timeout)
        last_error: Exception | None = None
        for attempt in range(_MAX_RETRIES + 1):
            try:
                request = requests.get if method.upper() == "GET" else requests.request
    ...
    def _load_all_sequences(self) -> List[Dict[str, Any]]:
        ...
        response = self._request(
            "GET", url, params={"all": "true"}, timeout=self.timeout
        )
