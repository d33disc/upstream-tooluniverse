...
class TestDependencyIsolationIntegration:
    ...
    def test_simulated_dependency_failure_integration(self):
        ...
        try:
            ...
            mark_tool_unavailable(
                "SimulatedTool1", ImportError('No module named "torch"')
            )
            mark_tool_unavailable(
    ...
    def test_mixed_success_and_failure_scenario(self):
        ...
        try:
            ...
            mark_tool_unavailable(
                "FailedTool2", ImportError('No module named "admet_ai"')
            )
    ...
    def test_error_recovery_after_fix(self):
        ...
        try:
            ...
            mark_tool_unavailable(
                "RecoverableTool", ImportError('No module named "test"')
            )
    ...
    def test_lazy_loading_with_failures(self):
        ...
        try:
            ...
            mark_tool_unavailable(
                "LazyFailedTool", ImportError('No module named "test"')
            )
    ...
    def test_concurrent_tool_access_with_failures(self):
        ...
        Previously marked ``@pytest.mark.skip(reason="Requires GPU and heavy
        ML model loading")``. The test body itself does not need a GPU — only
        the unfiltered ``tu.load_tools()`` call did, because it would
        instantiate embedding/RAG tools during loading. Fix: restrict loading
        to safe HTTP-only tools via ``include_tools``, which exercises the
        concurrent-access code path without touching any ML models.
        """
        import threading

        tu = ToolUniverse()
        try:
            # GPU-free tools — same list used by the lazy-load cache tests.
            tu.load_tools(
                include_tools=[
                    "Wikipedia_search",
